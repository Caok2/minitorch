...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert 0.0 < sigmoid(a) <= 1.0, "Not strictly between 0.0 and 1.0"
    assert_close(1 - sigmoid(a), sigmoid(-a)), "One minus sigmoid is not the same as the sigmoid of the negative"
    assert_close(sigmoid(0.0), 0.5), "Sigmoid should be crossing 0 at 0.5"
    prev_sigmoid = sigmoid(-5)
    for i in range(-4, 5, 1):
        curr_sigmoid = sigmoid(i)
        assert curr_sigmoid > prev_sigmoid, f"Sigmoid({i}) was not greater than Sigmoid({i-1})"
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if lt(a,b) == 1.0 and lt(b,c) == 1.0:
        assert lt(a,c) == 1.0, "Less-than should be transitive"
...
def test_symmetric(a: float, b: float) -> None:
    ...
    """
    assert mul(a, b) == mul(b, a), f"{a}*{b} did not return the same as {b}*{a}"
